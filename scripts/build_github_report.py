...
def normalize_markdown(text: str) -> str:
    ...
    text = text.replace(
        ...
        "Итоговый аналитический отчёт по прогнозированию потребления алкоголя в России в пересчёте на банки Guinness.",
    )
    ...
    text = text.replace("где l_t — уровень, а b_t — тренд.", "где `l_t` — уровень, а `b_t` — тренд.")
    text = text.replace(
        "В терминах итогового прогноза ключевым числом является прогноз Y_hat_2030, а прикладной вывод по риску задаётся сравнением G_day_2030 с диапазоном D_low–D_high.",
        "В терминах итогового прогноза ключевым числом является прогноз `Y_hat_2030`, а прикладной вывод по риску задаётся сравнением `G_day_2030` с диапазоном `D_low`–`D_high`.",
    )
